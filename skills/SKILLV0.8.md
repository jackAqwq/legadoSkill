...
1. **先判定“真实播放链路”再写规则**
   - 听书站点不要只盯目录页 HTML。
   - 先查是否存在独立播放页、iframe 播放器、前端 API、播放器 bundle。
   - 如果站点已经把真实地址解析逻辑放进播放器脚本，优先跟播放器逻辑对齐，不要死守旧 DOM 规则。

2. **详情页/目录页拿到的服务器地址不一定是真播放节点**
   - 某些站点页面里会同时出现“控制/展示用服务器”和“真实音频分发服务器”。
   - 如果旧源拼出的音频地址 404，要检查是否误用了 `AsType=2`、展示服、封面服、默认服。
   - 应优先从前端解密后的服务列表里选择 `AsType=1` 的真实音频节点，再拼接解密后的相对路径。

3. **`assl` / `efi` 这类字段失效时，优先怀疑“前端又包了一层”**
   - 常见变化不是 AES 密钥变了，而是密文前后或中间被插入了 `sk`、版本片段、混淆标记。
   - 这类问题的特征是：旧 AES 参数看起来没变，但直接解密报错或结果乱码。
   - 处理顺序：
     1. 查播放器脚本里是否先裁剪/替换密文
     2. 再做 AES 解密
     3. 再选服务器和拼最终 URL
...
4. **悦听吧案例**
   - `assl` 不是纯净密文，页面里插入了一段 32 位 `sk`。
   - 播放器会用首字符 `charCodeAt(0)` 作为切割位置，把这 32 位 `sk` 取出后再解密。
   - 旧源直接解 `assl` 会失败，或解出错误服务器。
   - `efi` 解密后得到的是相对音频路径，不是完整 URL。
   - 最终可播地址应为：`真实音频服务器(AsType=1)` + `解密后的相对路径`。

5. **Rhino/Legado JS 兼容性要保守**
   - 优先使用 `var`、普通函数、正则 `replace(/.../g, ...)`。
   - 避免依赖不确定的 Java 扩展方法。
   - 如果报错提示 `JavaPackage java.xxx 不是函数`，说明该 helper 在当前 Legado 环境不存在，需要改回 `java.base64DecodeToByteArray`、`java.createSymmetricCrypto` 这类已验证可用的方法。

6. **前端展示优化经验也要同步进规则思路**
   - 听书详情页的播音信息要做“有值才显示”，不要空标签占位。
   - 发现页多分类标签要注意一行布局和点击后的真实跳转参数，样式修正后必须再校验分类请求是否仍然命中正确 URL。

7. **音频服务列表里可能混有“死节点”，不要只做静态挑选**
   - 某些站点的 `assl` 解密后会给出多个 `AsType=1` 音频节点，但其中一部分已经 404、超时，或者只对特定书有效。
   - 仅按 `BookIds`、`Ratio`、默认顺序静态选一个节点，容易出现“浏览器偶尔能播、Legado 稳定报 2004”。
   - 更稳的做法是：
     1. 目录阶段保存完整服务列表
     2. 正文阶段解出相对音频路径后，按 `BookIds` 命中优先、空 `BookIds` 兜底的顺序生成候选 URL
     3. 使用 `java.head(url, headersJson, timeout)` 轻量探测，选第一个真正可达的节点
     4. 如果站点签名可能失效，可同时生成“裸直链”和“带 token 直链”两套候选，再探测落点

8. **听书 `ruleContent` 可以直接返回“音频 URL + 请求头选项”**
   - Legado 听书播放最终也会走 `AnalyzeUrl`，因此 `ruleContent` 不一定只能返回裸 URL。
   - 需要强制 `User-Agent`、`Accept-Encoding` 等头时，可以直接返回：
     `音频地址,{\"headers\":{...}}`
   - 这样能把请求头明确嵌进最终播放链接，减少“书源 header 生效链路不确定”带来的排障成本。

9. **APP 接口源要先净化请求串，再做静态/动态地址双探测**
   - 这类听书源的 `baseUrl` 往往不是纯 URL，而是 `url,{headers...}` 形式的请求串。
   - 如果目录或正文兜底时直接拿这个字符串去重请求，很容易触发 `MalformedURLException`，或者静默拿空响应。
   - 更稳的做法是：
     1. 先用 `stripRequestUrl()` 取逗号前的纯 URL
     2. 再从纯 URL 提取 `/api/...` path
     3. `requestBusinessData(pathOrUrl)` 同时兼容绝对 URL、纯 path、请求串三种输入
     4. 对业务接口统一按 `businessStaticAddress -> businessDynamicAddress` 顺序重试
   - 目录规则建议优先链路：
     1. 先吃当前响应
     2. 空了就用当前 `baseUrl` 重拉
     3. 还空就按 `albumId` 直打 `/api/business/v1/album/tracks/{id}.json`

10. **听书源报 `ContentEmptyException`，优先排查“取链为空”而不是先怀疑播放器**
   - 对音频源来说，这个异常通常不是 ExoPlayer 真播不了，而是 `ruleContent` 最终返回了空串。
   - 常见原因不是接口彻底失效，而是：
     - 播放信息被包进了 `data`
     - 直链字段不再叫 `path`，可能变成 `url/playUrl/audioUrl/mediaUrl/fileUrl`
     - `apiUrl2` 返回结构从 `list[0].path` 变成了别的层级
   - 更稳的处理方式是：
     1. 先写 `hasPlayData()` 判断当前响应里是否已有可用播放信息
     2. 再写 `normalizePlayData()` 解包 `data`
     3. 再写 `pickDirectUrl()` 统一提取各种直链字段
     4. 最后才回退到 `accessPath + 签名头` 或二跳接口
   - 只要拿到了裸音频地址，记得统一补 `#isMusic=true#`，避免 Legado 按普通正文处理。

11. **同系听书源并行处理时，先锁定“当前源身份”和“当前文件”**
   - 海云、轻云、悦听吧这类源往往接口相似、字段相近，但并不等于可以混着改。
   - 在连续对话里频繁切源时，最容易出现的问题不是规则本身，而是把 A 源的元信息、发现页、搜索规则误写进 B 源。
   - 更稳的做法是：
     1. 开工前先确认当前目标文件路径
     2. 再确认 `bookSourceName / bookSourceUrl / bookSourceComment`
     3. 修改完成后再复查一次源名、文件名、规则链路是否仍然对应当前站点
   - 如果只是“借思路”，不要顺手把对方的源名、注释、字段命名一起迁过去。

12. **跨源移植优先迁“展示层”，不要轻易迁“播放层”**
   - 同系 APP 接口源常常可以共用发现页标签、搜索列表清洗、简介排版、详情展示这些 UI/展示逻辑。
   - 但播放链路、签名头、注册逻辑、接口 host、二跳解析通常是最容易站点私有化的部分。
   - 推荐迁移顺序：
     1. 先迁 `exploreUrl / ruleExplore / ruleSearch` 这类展示层
     2. 再迁清洗 helper，如 `cleanText()`、`normalizeExploreItem()`
     3. 最后才考虑是否复用 `ruleContent`、签名算法、登录注册逻辑
   - 这样能最大限度复用“好用性优化”，同时避免把本来稳定的播放链路带坏。

13. **搜索页和发现页尽量共用一套“归一化映射层”**
   - 如果搜索结果直接取原始字段，常见会出现 `<word>` 高亮残留、作者字段带标签、分类字段格式不统一。
   - 比起给每个字段单独打补丁，更稳的是先把原始数据映射成统一对象：
     `bookId / title / authorText / kindText / introText / coverUrl`
   - 然后让 `ruleExplore`、`ruleSearch` 都消费这套归一化对象。
   - 这样一来：
     1. 标题和作者清洗逻辑只写一次
     2. 搜索和发现页显示风格自动统一
     3. 后续修字段脏数据时，只需要改映射层

14. **脚本批量改 JSON 时，重点检查“顶层结构”和“模板占位符”**
   - 用 PowerShell 或其他脚本重写书源时，容易出现两类高频事故：
     - 顶层数组 `[...]` 被写成单对象 `{...}`，导入直接失败
     - `{{$.bookId}}`、`${bid}`、反引号模板字符串被宿主 shell 提前吃掉，导致规则看起来合法、运行时却取不到值
   - 批量改写后的最低检查项：
     1. 顶层仍是数组
     2. `@js` 里的 `{{...}}` 占位符还在
     3. 目标规则里的 `host / bid / path` 拼接没有被 shell 抹掉
     4. 重新导出后再用 JSON 解析器过一遍
   - 如果只是为了生成 `bookUrl`，优先用普通字符串拼接，少用会被外层脚本误伤的模板字面量。

## 性能优化经验增补

书源写好能用之后，仍有大量空间可以精炼 JS 代码逻辑、减少冗余计算。以下为实战验证过的优化手法，按收益从高到低排列，来自 3 个真实书源的优化实战：

### 一、jsLib 函数层面（悦听吧 案例）

**案例概况**：悦听吧.json — 传统 HTML 听书源，原始 11.6KB / 18 函数。通过 jsLib 精炼 + content 简化，文件缩减至 11.3KB（-3.3%），主要收益来自代码质量提升而非体积。

15. **删除仅一次调用包装的函数，直接内联**
   - 典型冗余：`function ytbAudioHeadersJson() { return JSON.stringify(ytbAudioHeaders()); }`
   - 这类函数没有抽象价值，只是多占一条函数定义。
   - 优化：所有调用点直接写 `JSON.stringify(ytbAudioHeaders())`，删掉函数定义。

16. **移除 `==` 比较前的冗余 `String()` 转换**
   - 例：`String(item.Type) == 'AAAA'` → `item.Type == 'AAAA'`
   - 例：`String(list[i].AsType) == '1'` → `list[i].AsType == 1`
   - 原因：Rhino/Legado 的 `==` 是弱类型比较，`'1' == 1` 为 true，无需显式 String()。
   - 每个 `String()` 调用都是一次函数调用开销，在循环中会被放大。

17. **精简 `parseInt` 与 `substring` 的多余组合**
   - 例：`es = parseInt(result.assl.substring(0, 1).charCodeAt(0), 10);`
   - 优化为：`es = result.assl.charCodeAt(0);`
   - 原因：`charCodeAt(0)` 已经返回 number，`parseInt(number, 10)` 是恒等变换；`substring(0,1)` 再取 `charCodeAt(0)` 等同于直接对原字符串操作。
   - 一次优化省掉 2 次函数调用（substring + parseInt）。

18. **简单字符串提取优先用 `indexOf`，避免正则引擎开销**
   - 例：从 URL 提取文件名
     ```javascript
     // 慢：每次调用启动正则引擎
     ...
     ```
   - 此函数在每章音频 URL 拼接时都会触发，正则开销累积可观。

19. **`Number(value || 0)` 精简为 `(Number(value) || 0)`**
   - 前者多一层逻辑或运算后再做 Number；后者利用 JS 的 `NaN || 0 === 0` 特性直接兜底。
   - 语义等价但少一次短路求值。

### 二、ruleContent 执行路径（悦听吧 + 轻云听书）

20. **缓存重复的 `java.get()` 调用**
   - `java.get('ytbBookId')` 若在同一 `@js:` 块被调用两次（如分别用于 `bookId` 和 `cacheBookId` 兜底），应先把结果存入局部变量：
     ```javascript
     var cachedBookId = java.get('ytbBookId');
     var bookId = item.bookId || cachedBookId || '';
     var cacheBookId = java.get('ytbCacheBookId') || cachedBookId || '';
     ```
   - 每减少一次 `java.get()`，就少一次 Java/Native 桥接开销。

21. **利用 `ytbEnsureBookPageLoaded` 的语义简化缓存校验**
   - 该函数返回后，缓存一定已是当前书籍的最新状态。
   - 因此不需要在 content 中再次检查 `cacheBookId == bookId`。
   - 直接读 `java.get('ytbAudioBaseUrl')`，若为空才回退到从 serverList 挑选。
   - 省掉 `var cacheBookId` 和 `var cacheValid` 两个变量及对应条件分支。

22. **提取重复条件为中间变量，合并相邻 `if` 块**
   - 原代码中两个 `if` 有完全相同的条件表达式：
     ```javascript
     // 优化前：条件重复计算
     if (cond) { baseUrl = java.get(…); }
     if (!baseUrl && cond) { /* fallback */ }
     ```
   - 优化后：
     ```javascript
     var cacheValid = cond;
...
23. **临时变量能内联就内联**
   - `var headersJson = …; result = url + headersJson;` → `result = url + (…);`
   - 减少一次变量声明和赋值，对 Rhino 内存分配有微益。

### 三、自动化改写注意事项（悦听吧 + 轻云听书）

24. **用脚本改 JSON 时注意转义陷阱**
   - PowerShell `-replace` 是正则替换，`.`、`?`、`$`、`(`、`)`、`[`、`]`、`+`、`*` 都需反斜杠转义。
   - 尤甚当待替换内容本身是 JS 正则字面量（如 `/\/([^\/?#]+)/`），转义层数翻倍极易写错。
   - 更稳方案：对已知精确文本用 `.Replace()`（字面量替换）代替 `-replace`（正则替换）。
   - 用 here-string（`@'...'@`）写大段 JS 时，先 `-replace "\`r\`n", "\`n"` 统一换行符为 LF。

25. **批改后必须逐项验证，不可只看总长度**
   - 文件变小不意味着所有改动都生效。每项优化后单独验证：
     ```
     $lib -match 'parseInt'        # 应为 false
     $lib -match 'String\(item'    # 应为 false
     $lib -match 'lastIndexOf'     # 应为 true
     ```
   - 逐项比对能第一时间发现"某步替换静默失败"。

26. **`.Replace()` 字面量替换注意同名文本陷阱**
   - PowerShell `.Replace(old, new)` 会替换**所有**出现位置，不像正则可以锚定上下文。
   - 典型事故：`$lib.Replace("    return legadoJava;`n", "")` 会同时删除 `bindEnv` 和 `getLegadoJava` 中的同名 `return` 行，导致 `getLegadoJava()` 返回 `undefined` → 所有 `md5Encode` 调用报错。
   - 更稳做法：
     1. 用函数级别的完整文本（含函数签名）做 `.Replace()`，确保唯一性
     2. 或用正则 `-replace` 锚定到特定函数体内
     3. 替换后必须提取目标函数全文验证完整性

### 四、死代码清理与函数迁移（轻云听书 案例）

**案例概况**：轻云听书.json — API 接口型听书源，原始 29.5KB / 48 函数 / 602 行 jsLib。通过 3 轮优化缩减至 27.0KB（-8.5%），content 从 5,657 → 2,013 字符（-64%）。

| 轮次 | 改动 | 收益 |
|------|------|------|
| 1 | 删除 5 个无调用死函数（`canHeadUrl` `getDoubleMd5` `buildSelfPlayContent` `resolvePlayContentData` `resolvePlayContent`） | jsLib -2,301 字符 |
| 2 | `.Replace()` 同名文本陷阱修复：删 `bindEnv` 的 return 时误删了 `getLegadoJava` 的 return | 修复后 `md5Encode` 调用链恢复 |
| 3 | content 中 5 个内联函数（`hasPlayData` `normalizePlayData` `appendMusicFlag` `pickDirectUrl` `buildSelfResult`）迁入 jsLib | content -64% |

27. **死代码识别三问法**
   - 对 jsLib 中每一个函数，依次检查：
     1. 谁调用它？（直接在 jsLib 全文搜索函数名）
     2. 调用者是否绝对死锁？（继续向上追溯调用链）
     3. 它的功能是否已被其他函数替换？
   - 示例：`resolvePlayContent` → `resolvePlayContentData` → `buildSelfPlayContent` 整条链无任何规则引用，且 `buildSelfPlayContent` 的 `$$` 签名行为与 content 内 `buildSelfResult` 不同，属于已废弃的历史遗留代码。
   - 附加检查：查看 `bookSourceComment` 注释（如 `"添加缺失的canHeadUrl函数"`），如果注释说添加了但规则中找不到调用，大概率也是死代码。

28. **content 内联函数迁移到 jsLib 的标准流程**
   - 前提：content `@js:` 块内的函数定义会**每章重新解析**，迁移到 jsLib 则只在书源加载时解析一次。
   - 迁移步骤：
     1. 识别 content 中的纯函数（无闭包依赖，或依赖可通过 `ensureConfig()`/`getLegadoJava()` 自给自足）
     2. 对闭包依赖函数（如 content 中 `buildSelfResult` 依赖 `javaHelper`/`appService`/`update`），重构为自给自足版本：
        - `javaHelper` → 内部调用 `getLegadoJava()`
        - `appService`/`update` → 内部调用 `ensureConfig().appService/.update`
     3. jsLib 尾部追加新函数（保持原有函数顺序不变，降低风险）
     4. content 中：
        - 删除函数定义
        - 统一函数名引用（如 `buildSelfResult` → `buildSelfPlayUrl`）
        - 删除仅被迁出函数使用的局部变量（如 `cfg`/`appService`/`update`）
     5. 验证：`content -split 'function '` 计数应为 0（完整迁移时）
   - 收益示例：轻云听书 content 从 5,657 → 2,013 字符（-64%），5 个内联函数全部迁入 jsLib。

29. **跨脚本批改必须单批次完成**
   - PowerShell 分两次脚本执行时，第一次脚本对变量的修改不会自动写回文件。
   - 第二脚本从磁盘读取时拿到的是旧内容，导致前一轮改动丢失。
   - 正确做法：将所有改动放在同一个 `$ErrorActionPreference = "Stop"` 脚本中，按序完成"读取 → 修改 → 保存"。

30. **优化后必须验证「被保留函数」的完整性**
   - 删除死代码后，必须验证被保留的关键函数是否完整：
     ```
     $lib -match 'buildLanrenPlayContent'     # 应为 True（被 content 调用）
     $lib -match 'buildExploreTabs'          # 应为 True（被 exploreUrl 调用）
     $lib -match 'requestBusinessData'       # 应为 True（被 toc 规则调用）
     ```
   - 特别注意：删除函数 A 前，确认 A 调用的子函数 B 是否**只**被 A 使用。如果 B 也被其他活跃函数使用，删除 A 后必须保留 B。
   - 示例：`getDoubleMd5` 只被 `buildSelfPlayContent` 调用 → 两者一起删。`buildLanrenPlayContent` 被 `resolvePlayContentData`（死代码）和 content 规则（活跃）调用 → 只删前者，保留后者。

### 五、Nuxt SSR / WebView 书源经验（听友FM 案例）

31. **Nuxt SSR 站点不需要 CSS 选择器 — 直接解 payload**
   - 现代前端框架（Nuxt/Next）常把数据嵌入 `window.__NUXT_DATA__` 或渲染为内联 `<script id="__NUXT_DATA__">`。
   - 核心函数：`parsePayload(rawHtml)` — 先检测是否为纯 JSON 字符串，否则正则提取 `<script>` 标签内容，再经 `reviveNuxt` 递归反序列化（处理 `ShallowReactive`/`Reactive`/`Ref`/`Set` 等 Vue 包装类型）。
   - 这套 payload 解析链路一次写成后，搜索、发现、详情、目录全部复用，规则代码极度精简。

32. **WebView 播放模式的 ruleContent 写法**
   - 对需要浏览器会话（cookie/sessionStorage）的站点，`ruleContent` 可以极简：
     ```json
     {"content": "<js>result</js>", "webJs": "(function(){...})()"}
     ```
   - `webJs` 在 WebView 中执行，负责调用站点的前端 API（如 `window.$fetch`/`window.__checkSession`），将最终播放地址写入 `window.__legadoPlayUrl`。
   - `ruleContent: <js>result</js>` 直接把 webJs 的结果透传。
   - 注意点：
     - WebView 启动有开销，`respondTime` 需放宽（本源 180s）
     - 目录链接需附加 `,{\"webView\":true,\"webViewDelayTime\":1000}` 确保 session 预热
     - `webJs` 中必须有并发保护（`__legadoBusy` 标记），防止重复请求

33. **Nuxt payload 的数据访问模式**
   - `reviveNuxt` 反序列化后得到嵌套对象，数据路径通常为：
     ```
     payload.data['album-detail-' + albumId]       // 专辑详情
     payload.data['album-chapters-' + albumId]      // 章节列表
     payload.data.filters.categories[].types[]      // 分类元数据
     payload.data['index-home-tabs'][tabKey].items  // 首页标签数据
     ```
   - 通过动态 key 拼接（`'album-detail-' + albumId`）访问不同专辑数据，避免额外 HTTP 请求。
   - `ruleBookInfo.init` 在一条响应中同时解析详情、章节数、最后一章标题，一次性输出所有字段。

34. **轻量书源的优化策略 — 跨规则重复模式提取**
   - 对于本身紧凑的书源（12KB 以下），死代码通常不存在，优化重点是**跨规则重复模式**：
     ```
     albumId 提取: /albums/(\d+)/  → 出现在 bookInfo、toc、webJs 共 3 处
     baseUrl 模式: String(baseUrl || '') → 出现在 bookInfo、toc、explore 共 3 处
     ```
   - 提取为 jsLib 函数（如 `extractAlbumId(url)`）后可减少每条规则 ~40 字符，3 处共省 ~120 字符。
   - 这种优化的**代码质量收益 > 文件体积收益**，但减少了未来修改时多端遗漏的风险。

35. **发现页的双路径模式（分类 + 标签）**
   - `ruleExplore.bookList` 中同时处理两种页面：
     - 分类页（`/categories/{id}`）：从 `filters.categories` 元数据中查找分类名
     - 首页标签（`?legadoTab=latest`）：从 `index-home-tabs` 索引区取对应区块
   - 关键技巧：通过正则 `/\/categories\/(\d+)/` 判断当前请求类型，分支处理。
   - 分类名查找使用双重嵌套循环 + `break outer` 跳出，比 `Array.find` + `Array.some` 在 Rhino 中更可靠。

36. **IIFE 内联构建 vs jsLib 函数提取**
   - `ruleBookInfo.init` 中使用 `(function() { ... })()` 在内联上下文中构建 author 字符串：
     ```javascript
     author: (function() {
       var parts = []; var a = clean(detail.author);
       var t = clean(detail.teller);
       if (a) parts.push('作者：' + a);
       if (t) parts.push('播音：' + t);
       return parts.join('  ');
     })()
     ```
   - 这种 IIFE 能避免污染外部作用域，但若逻辑被多规则复用（如 explore 的 `mapAlbumItem` 已有类似 author 拼接），应提取为 `buildAuthorText(author, teller)` 放入 jsLib。
   - 判断标准：同一逻辑出现 ≥2 次 → 提取；仅 1 次 → IIFE 即可。

37. **防御性代码谨慎删除**
   - `uniqueLines` 中检测字符串自我重复（`"abcabc" → "abc"`）的逻辑看似多余，但实际是对 Nuxt payload 中特定 artifact 的防御。
   - 这类"看起来没用但实际有用"的代码特征是：逻辑极具体（`length % 2 === 0` + 半段对比），不是泛化抽象。
   - 优化原则：**不理解的防御性代码不要删**，它背后可能有踩过的坑。
...
很多新站点的发现页不是传统 HTML 列表，而是 **SSR + 前端 hydration**，这类站点要优先判断“样式问题”和“数据问题”分别归谁管。

**吸收内容**：
1. **发现标签的排版样式归 Legado 客户端控制，不归网站 HTML 控制**
   - 如果用户要“三个标签一行”“标签文字对齐”，优先查看 Legado 的 `ExploreKind.style / FlexChildStyle`
   - 可以在 `exploreUrl:@js` 中直接返回对象数组：`{title, url, style}`
   - 三列布局常用写法：
```json
{
  "title": "最新",
  "url": "/?tab=latest&page={{page}}",
  "style": {
    "layout_flexGrow": 1,
    "layout_flexShrink": 1,
    "layout_alignSelf": "auto",
    "layout_flexBasisPercent": 0.31,
    "layout_wrapBefore": false
  }
}
```

2. **发现页点击后如果报 `TimeoutCancellationException: Timed out waiting for 60000 ms`，先排查是不是发现列表走了 `webView:true`**
   - 这类超时通常是 `BackstageWebView.getStrResponse()` 外层等待超时
   - **不一定**是 `webJs` 本身执行超时
   - 说明页面加载回调没有按预期完成，继续死磕 WebView 往往收益很低

3. **对 Nuxt / SSR / 内嵌 payload 站点，发现页优先“去 WebView 化”**
   - `exploreUrl` 只负责输出发现标签和普通分页 URL
   - `ruleExplore.bookList` 改用 `@js:` 直接解析 HTTP 返回 HTML 中的 `__NUXT_DATA__`、`window.__NUXT__` 或其他内嵌 payload
   - 只有当列表数据必须等待前端异步请求、且原始响应里完全拿不到时，才退回 WebView 方案

4. **`bookList:@js` 可以直接返回对象数组**
   - 返回值可直接是 `[{name, author, kind, bookUrl, coverUrl}]`
   - 字段规则优先使用 plain key：`name`、`author`、`bookUrl`
   - 不要先入为主写成 `$.name`、`$.author`
   - 只有确认返回值是 JSON 字符串或必须走 JSONPath 时，再使用 `$[*] / $.field`
...
**适用场景**：
- 站点本质是 APP 接口，不适合抓网页 DOM
- 搜索、发现、详情、目录、播放地址都来自接口
- 接口返回 `encData`、签名头、动态时间戳、播放二跳等内容

**核心模式**：
1. 把公共能力集中到 `jsLib`
   - `creatRequest()`：统一拼请求头、时间戳、签名
   - `decrypt()`：统一解密 `encData`
   - `api()`：可选，切换备用接口
2. 搜索和发现先解密，再吐数据
   - 如果字段规则准备用 JSONPath，`bookList:@js` 可以返回 `items.map(x => JSON.stringify(x))`
   - 如果后续字段准备直接取对象键，也可以直接返回对象数组
3. 详情、目录、正文按 API 链逐层跳转
   - `bookUrl` 先跳详情接口
   - `tocUrl` 再跳目录接口
   - `chapterUrl` 再跳播放分析接口
4. 正文/播放规则要允许多分支
   - 一种情况直接返回音频地址 + headers
   - 另一种情况继续请求二跳接口拿真实播放地址
5. 复杂接口源最好带运维按钮
   - `loginUi` + `loginUrl`：重新注册、刷新 device_id
   - 自定义按钮：切换备用 host、清缓存、改接口

**最小骨架**：
```json
{
  "jsLib": "function creatRequest(url, method, body){...}\nfunction decrypt(data){...}\nfunction api(){...}",
  "searchUrl": "@js:\ncreatRequest(host + '/search', 'POST', {key:key, page:page})",
  "ruleSearch": {
    "bookList": "@js:\nlet encData = JSON.parse(result).body.encData;\nJSON.parse(decrypt(encData)).items.map(function(x){return JSON.stringify(x)})",
    "name": "$.title",
    "author": "$.announcerName",
    "bookUrl": "@js:\ncreatRequest(host + '/album/info/{{$.albumId}}.json')"
  },
  "ruleBookInfo": {
    "init": "@js:\nJSON.parse(decrypt(JSON.parse(result).body.encData))",
    "tocUrl": "@js:\ncreatRequest(host + '/album/tracks/{{$.id}}.json')"
  },
  "ruleToc": {
    "chapterList": "@js:\nJSON.parse(decrypt(JSON.parse(result).body.encData)).map(function(x){return JSON.stringify(x)})",
    "chapterUrl": "@js:\ncreatRequest(host + '/album/analysis/{{bid}}/{{$.id}}.json')"
  },
  "ruleContent": {
    "content": "@js:\nvar data = JSON.parse(decrypt(JSON.parse(result).body.encData));\nif (data.accessPath) {\n  result = data.accessPath + ',' + JSON.stringify({headers:{...}});\n} else {\n  result = java.get(...).body();\n}\nresult;"
  }
}
```

**关键提醒**：
- 学的是“分层结构”，不是照抄别人源里的 IP、盐值、appId、签名常量
- 复杂音频源的稳定性，往往取决于 `jsLib` 是否把请求、解密、签名、切接口统一封装
- 如果播放接口存在多分支，不要强行写成单一路径
...
##### 1. 海云听书型音频源的 jsLib 作用域与导入结构踩坑修正

**用户反馈**：优化后的“海云听书”源在 Legado 中先后出现两类真实问题：
- 运行时报错：`java.strToBytes` 不是函数
- 导入时报错：`Expected BEGIN_ARRAY but was BEGIN_OBJECT`

**吸收内容**：
- `jsLib` 运行在共享作用域时，不能想当然依赖 `this.java` 或 `const { java } = this`
- 对共享 `jsLib` 的复杂源，更稳的方式是在每个 `@js` 入口先显式 `bindEnv(java)`，再让公共函数通过缓存变量取 `java`
- 书源 JSON 导入给 Legado 时，顶层必须是数组 `[{...}]`，不能是单对象 `{...}`
- 优化一个“原本可用”的书源时，先做结构校验，再做运行时校验；不要只看本地 JSON 解析通过
- Rhino 兼容性继续保持保守：不要回流 `?.` 这类不稳语法
...
##### 10. 现代前端站点发现页三列样式与去 WebView 化（重要新增）

**用户反馈**：发现页想做成三个标签一行，样式改对后点击标签却提示加载失败，日志是 `TimeoutCancellationException: Timed out waiting for 60000 ms`

**问题分析**：
- 一开始把问题当成网站前端样式问题处理，但真正控制发现标签布局的是 Legado 客户端
- 后续为了让分类页可用，把发现页标签链接拼成了 `webView:true + webJs`，结果点击标签时走进了 `BackstageWebView.getStrResponse()` 的 60 秒外层超时
- 这说明发现页列表并不适合继续依赖 WebView 回调，应该改成普通请求 + 解析 SSR payload

**吸收内容**：
1. 发现页三列布局优先使用 `ExploreKind.style / FlexChildStyle`
2. `exploreUrl:@js` 可以直接返回带 `style` 的对象数组来控制发现标签排版
3. 如果发现页点击后是 60 秒超时，优先怀疑 `webView:true` 链路，而不是先继续改 `webJs`
4. 对 Nuxt / SSR 站点，优先在 `ruleExplore.bookList:@js` 中解析原始 HTML 里的 `__NUXT_DATA__`
5. `bookList:@js` 返回对象数组后，字段规则优先写 plain key，而不是 JSONPath
6. 只有原始 HTML 真拿不到列表数据时，才把发现页退回 WebView 方案

**正确流程**：
```
发现页要三列
    ↓
先查 Legado 的 ExploreKind.style / FlexChildStyle
    ↓
exploreUrl:@js 返回 {title, url, style}
    ↓
点击标签超时
    ↓
先移除 webView:true
    ↓
ruleExplore.bookList:@js 解析 HTML 内嵌 payload
    ↓
字段规则用 plain key 收尾
```
...
##### 11. 海云听书型音频接口源的分层写法（重要新增）

**用户反馈**：提供了一个可用的“海云听书”书源，希望吸收其中能复用的写源经验

**问题分析**：
- 这类源真正厉害的不是某个字段规则，而是把“请求构造、签名、解密、注册、接口切换、播放二跳”全部做了分层
- 如果只抄 `ruleSearch` 或 `ruleContent`，很快会在别的站点失效
- 真正可迁移的是它的架构：`jsLib` 做基础设施，规则层只做调用和串联

**吸收内容**：
1. 复杂音频源优先按“基础设施层 + 规则层”拆分
2. `jsLib` 里统一收口 host、请求头、时间戳、签名、解密函数
3. 搜索/发现如果返回加密数组，`bookList:@js` 可以先解密，再 `map(JSON.stringify)` 给 JSONPath 用
4. 详情、目录、正文应按接口链逐层跳，不要试图一条规则直接打穿全部流程
5. 播放规则要允许“直出音频地址”和“二跳解析”两种分支共存
6. 对易失效的 APP 接口源，建议加入 `loginUi/loginUrl` 或切换接口按钮，降低维护成本
7. 学习时提炼方法论，不照抄站点私有常量、IP、盐值、设备注册参数
...
##### 12. 海云听书型音频源的真实踩坑修正：jsLib 作用域与导入结构（重要新增）

**用户反馈**：在“海云听书”可用源基础上做保守优化后，真实测试里出现了两个典型问题：
- 运行时报错：`TypeError: 无法调用对象 [JavaPackage java] 中的属性 strToBytes`
- 导入时报错：`Expected BEGIN_ARRAY but was BEGIN_OBJECT`

**问题分析**：
- `jsLib` 被放进共享作用域后，函数内部的 `this` 不等于每次 `@js` 执行时的运行时绑定；这里继续用 `this.java` 或 `const { java } = this`，很容易拿到 `JavaPackage java` 而不是 Legado 注入的 helper
- PowerShell / JSON 重写过程中，如果把顶层数组误写成单对象，本地看起来仍是合法 JSON，但 Legado 会直接拒绝导入
- 这类问题不是“解密算法错了”，而是“运行时作用域”和“导入结构”错了，排查顺序必须调整

**吸收内容**：
1. 对共享 `jsLib` 的复杂源，优先使用“入口显式绑定 + 公共函数取缓存”的模式
2. 推荐写法：在每个会调用公共函数的 `@js` 规则开头先 `bindEnv(java);`
3. `jsLib` 内部统一通过 `getLegadoJava()` 取 helper，不再使用 `this.java`、`const { java } = this`
4. 优化现有可用源时，交付前至少检查三件事：
   - 顶层是否仍是 JSON 数组 `[...]`
   - 是否残留 `this.java` / `const { java } = this`
   - 是否混入 Rhino 不稳语法，如 `?.`
5. 如果“原源可用、优化后报错”，先怀疑运行时绑定和文件结构，不要第一时间怀疑签名、盐值、解密逻辑
...
公共能力收进 jsLib
    ↓
每个 @js 入口先 bindEnv(java)
    ↓
jsLib 公共函数只从缓存 helper 取 java
    ↓
导出前检查顶层必须是数组 [...]
    ↓
再去 Legado 里做真实导入和运行测试
```
...
##### 13. 轻云听书型 APP 接口源的目录/正文三级兜底（重要新增）

**用户反馈**：同一个轻云听书源里，先后出现了“有的目录获取失败”和“有的播放点击后提示获取资源链接出错 / `ContentEmptyException: 内容为空`”。

**问题分析**：
- 目录和正文虽然都走业务接口，但失败形态不同，不能只修一边。
- 这类源的 `baseUrl` 在 Legado 里常常是 `url,{headers...}` 请求串；如果兜底重拉时不先剥离请求选项，会把非法字符一起带进 URL。
- 目录接口有时当前响应为空，但同 path 在静态/动态业务地址中的另一个节点是有数据的。
- 正文接口有时并不是彻底失效，而是播放信息被包进 `data`，或者直链字段名发生了变化，导致 `ruleContent` 最终返回空串，进而抛出 `ContentEmptyException`。

**吸收内容**：
1. 对同系 APP 接口源，`requestBusinessData()` 要设计成同时接受“请求串 / 绝对 URL / 纯 path”。
2. 兜底请求前必须先 `stripRequestUrl()`，再提取 `/api/...` path，避免 `MalformedURLException`。
3. 目录规则不要只依赖当前响应；推荐链路是“当前响应 -> baseUrl 重拉 -> `/album/tracks/{id}.json` 直打”。
4. `ruleContent` 不能只认 `accessPath + playType`，还要兼容 `data` 包裹和多种直链字段名。
5. 听书源出现 `ContentEmptyException` 时，先检查“取链是否为空”，不要先把问题归咎到播放器。
6. 对可直接播放的直链字段，优先直接返回并补 `#isMusic=true#`；只有直链拿不到时，再回退签名头或二跳接口。
...
先看当前响应是否有列表
    ↓
没有就 stripRequestUrl(baseUrl)
    ↓
按 static / dynamic 双地址重拉
    ↓
还没有就直打 /album/tracks/{id}.json

正文报 ContentEmptyException
    ↓
先判定 ruleContent 是否返回空串
    ↓
解包 data / list
    ↓
统一扫描 path/url/playUrl/audioUrl/mediaUrl/fileUrl
    ↓
命中直链先返回
    ↓
最后才回退 accessPath 或 apiUrl2
```
...
##### 14. 同系听书源连续迭代时的上下文锁定与展示层迁移（重要新增）

**用户反馈**：在连续调试轻云、海云两个同系听书源时，希望把轻云已经验证可用的发现页和搜索易用性优化迁移到海云，同时要求不要把源本身写乱。

**问题分析**：
- 同系源接口长得像，很容易在连续会话里“思路借对了，文件改错了”。
- 真正适合横向迁移的，往往是发现页标签、搜索清洗、详情排版这些展示层逻辑，而不是播放链路。
- 用脚本批量改 JSON 时，除了规则正确性，还要防止顶层数组丢失、模板占位符被外层脚本吞掉。

**吸收内容**：
1. 连续处理多个同系书源时，先锁定当前目标文件和当前源身份，再动规则。
2. 跨源迁移优先迁展示层：`exploreUrl / ruleExplore / ruleSearch / 清洗 helper`。
3. 搜索和发现页最好共用一套归一化映射层，统一清理 `<word>`、作者标签、分类脏数据。
4. 播放链路、签名、注册、登录逻辑默认视为站点私有，不因“看起来相似”就直接复用。
5. 批量重写 JSON 后，必须检查顶层数组、模板占位符和源名元信息是否仍然正确。
...
##### 15. 遇到可用参考版时，优先做“差异回退”，不要继续猜播放层（重要新增）

**用户反馈**：连续修悦听吧后一直报 `ERROR_CODE_IO_BAD_HTTP_STATUS 2004`，随后提供了一个可用版书源让我们参考。

**问题分析**：
- 连续几轮修复都围绕“节点探测、token、请求头”做推测性微调，但错误形态没有变化。
- 对这类听书源，若用户已经给出“当前站点真实可用”的参考版，继续重写播放链路的收益通常低于直接对比回退。
- 真正高风险的通常不是搜索和发现页，而是播放三件套：`jsLib`、`ruleToc.chapterList`、`ruleContent.content`。

**吸收内容**：
1. 当用户提供已验证可用的参考版时，优先逐项 diff 当前源与参考版。
2. 对听书源，首先比对并必要时整体回退这三块：
   - `jsLib`
   - `ruleToc.chapterList`
   - `ruleContent.content`
3. 如果连续修改后仍报同一个播放错误，不要继续围绕同一假设微调，先恢复稳定播放链路。
4. “从前端脚本推导出的更优实现”优先级低于“用户已验证可用的现成实现”。
5. 回退到可用版后，后续优化默认只动展示层，不再顺手改播放层。
...
拿可用参考版做 diff
    ↓
优先核对 jsLib / ruleToc / ruleContent
    ↓
先恢复稳定播放
    ↓
再决定是否只做展示层优化
...
##### 16. `chapterUrl` 的 `@js` 可能跑在主线程，禁止直接发网络请求（重要新增）

**用户反馈**：把网络请求塞进章节链接的 `@js` 后，真实报错变成了 `org.mozilla.javascript.WrappedException: Wrapped android.os.NetworkOnMainThreadException`。

**问题分析**：
- 并不是所有 `@js` 执行点都允许安全发网络请求。
- `chapterUrl` 这类取值点在当前链路下可能运行在主线程，直接调用 `java.get/java.post/java.connect` 会触发主线程网络异常。
- 这类错误和站点是否可播无关，是规则放置位置选错了。

**吸收内容**：
1. 不要在 `chapterUrl` 的 `@js` 里直接发网络请求，尤其不要调用 `java.get/java.post/java.connect`。
2. `chapterUrl` 更稳的职责是只返回普通 URL / API 地址 / 已有数据字段，不做联网二跳。
3. 需要联网取链、解密、拼最终音频地址时，优先放到 `ruleContent`。
4. 如果某个 `@js` 执行点是否允许联网不确定，默认按“不允许”处理，先查源码再动手。
...
##### 17. 已验证可播的听书源，默认冻结播放层，只优化展示层（重要新增）

**用户反馈**：书源最后决定“先这样”，重点是保持可用，不再继续折腾播放。

**问题分析**：
- 对已验证可播的听书源，播放链路的稳定性价值高于局部“优化感”。
- 真正低风险、可持续的改进通常在搜索、发现、详情展示，而不是播放层。

**吸收内容**：
1. 只要听书源已验证可播，默认冻结以下区域：
   - `jsLib`
   - `ruleToc.chapterList`
   - `ruleContent.content`
2. 优化优先级改为：
   ...
   - `ruleSearch`
   - `ruleBookInfo`
3. 如果必须实验播放层，先复制稳定版做副本，不覆盖原可用文件。
4. 交付前建议再次和稳定版对比播放三件套，确认没有误改。
